...
interface Step { id: number; title: string; description: string; details: string[]; duration: string; }
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="flex-1">
              <div className="mb-8 p-5 rounded-xl bg-card border border-border">
                ...
                <p className="text-xs text-muted-foreground mt-2">{completedSteps.length}/{steps.length} bước hoàn thành</p>
              </div>
              ...
              <div className="space-y-4">
                {steps.map((step) => {
                  ...
                  return (
                    <motion.div key={step.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: step.id * 0.05 }} className={`rounded-xl border transition-all ${isCompleted ? "border-green-500/30 bg-green-500/5" : isExpanded ? "border-gold/30 bg-card" : "border-border bg-card"}`}>
                      <button onClick={() => setExpandedStep(isExpanded ? null : step.id)} className="w-full flex items-center gap-4 p-5 text-left">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className={`text-sm font-medium ${isCompleted ? "text-green-400 line-through" : "text-foreground"}`}>Bước {step.id}: {step.title}</h3>
                          <p className="text-xs text-muted-foreground mt-0.5">{step.description}</p>
                        </div>
                        <span className="text-xs text-muted-foreground/60 shrink-0">{step.duration}</span>
                          ...
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                            <div className="px-5 pb-5 pt-0">
                              <div className="pl-14 space-y-2">
                                ...
                                <button onClick={(e) => { e.stopPropagation(); toggleComplete(step.id); }} className={`mt-3 inline-flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-medium transition-colors ${isCompleted ? "bg-green-500/10 text-green-400 hover:bg-green-500/20" : "bg-primary/10 text-gold hover:bg-primary/20"}`}>
